...
RSpec.describe Lite::Address::Details do
  ...
  it("line1 with valid addresses") do
  ...
  it("line1 with intersections") do
  ...
  it("line1 with informal addresses") do
  ...
  it("line2 with valid addresses") do
    ADDRESSES.each_pair do |address, expected|
      addr = Lite::Address::US.parse(address)
  ...
  it("line2 with intersections") do
    INTERSECTIONS.each_pair do |address, expected|
      addr = Lite::Address::US.parse(address)
      expect(expected[:line2]).to(eq(addr.line2))
    end
  ...
  it("line2 with informal addresses") do
    INFORMAL_ADDRESSES.each_pair do |address, expected|
      addr = Lite::Address::US.parse_informal_address(address)
      expect(expected[:line2]).to(eq(addr.line2))
    end
  ...
  it("to s with valid addresses") do
    ADDRESSES.each_pair do |address, expected|
      addr = Lite::Address::US.parse(address)
      expected_result = expected[:to_s]
      expected_result ||= [expected[:line1], expected[:line2]].select { |l| (not l.empty?) }.join(", ")
      expect(expected_result).to(eq(addr.to_s))
    end
  end

  it("to s with intersections") do
    INTERSECTIONS.each_pair do |address, expected|
      addr = Lite::Address::US.parse(address)
      expected_result = expected[:to_s]
      expected_result ||= [expected[:line1], expected[:line2]].select { |l| (not l.empty?) }.join(", ")
  ...
  it("to s with informal addresses") do
    INFORMAL_ADDRESSES.each_pair do |address, expected|
      addr = Lite::Address::US.parse(address)
      expected_result = expected[:to_s]
      expected_result ||= [expected[:line1], expected[:line2]].select { |l| (not l.empty?) }.join(", ")
  ...
  it("to s with no line2") do
    address = "45 Quaker Ave, Ste 105"
    addr = Lite::Address::US.parse(address)
    expect("45 Quaker Ave Ste 105").to(eq(addr.to_s))
  end

  it("to s with valid addresses with zip ext") do
    address = "7800 Mill Station Rd Sebastopol CA 95472-1234"
    addr = Lite::Address::US.parse(address)
    expect("7800 Mill Station Rd, Sebastopol, CA 95472-1234").to(eq(addr.to_s))
  end

  it("to s for line1") do
    address = "7800 Mill Station Rd Sebastopol CA 95472-1234"
    addr = Lite::Address::US.parse(address)
    expect(addr.line1).to(eq(addr.to_s(:line1)))
  end

  it("to s for line2") do
    address = "7800 Mill Station Rd Sebastopol CA 95472-1234"
    addr = Lite::Address::US.parse(address)
    expect(addr.line2).to(eq(addr.to_s(:line2)))
  end

  it("full postal code") do
    address = "7800 Mill Station Rd Sebastopol CA 95472-1234"
    addr = Lite::Address::US.parse(address)
