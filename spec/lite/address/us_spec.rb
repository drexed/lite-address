...
RSpec.describe Lite::Address::US do
  ...
  it('expected failures') do
    EXPECTED_FAILURES.each do |address|
      parsed_address = described_class.parse(address)
      expect((!parsed_address or !parsed_address.state)).to(be_truthy)
    end
  end

  it('street type is nil for road redundant street types') do
    address = '36401 County Road 43, Eaton, CO 80615'
    expected_results = { number: '36401', street: 'County Road 43', city: 'Eaton',
                         state: 'CO', postal_code: '80615', street_type: nil }
    parsed_address = described_class.parse(address, avoid_redundant_street_type: true)
  ...
  it('informal parse normal address') do
    a = described_class.parse('2730 S Veitch St, Arlington, VA 222064444', informal: true)
  ...
  it('informal parse informal address') do
    a = described_class.parse('2730 S Veitch St', informal: true)
    expect(a.number).to(eq('2730'))
    expect(a.prefix).to(eq('S'))
    expect(a.street).to(eq('Veitch'))
    expect(a.street_type).to(eq('St'))
  ...
  it('informal parse informal address trailing words') do
    a = described_class.parse('2730 S Veitch St in the south of arlington', informal: true)
    expect(a.number).to(eq('2730'))
    expect(a.prefix).to(eq('S'))
    expect(a.street).to(eq('Veitch'))
    expect(a.street_type).to(eq('St'))
  ...
  it('parse') do
    ...
    parseable = ['1600 Pennsylvania Ave Washington DC 20006',
                 '1600 Pennsylvania Ave #400, Washington, DC, 20006', '1600 Pennsylvania Ave Washington, DC', '1600 Pennsylvania Ave #400 Washington DC', '1600 Pennsylvania Ave, 20006', '1600 Pennsylvania Ave #400, 20006', '1600 Pennsylvania Ave 20006', '1600 Pennsylvania Ave #400 20006', 'Hollywood & Vine, Los Angeles, CA', 'Hollywood Blvd and Vine St, Los Angeles, CA', 'Mission Street at Valencia Street, San Francisco, CA', 'Hollywood & Vine, Los Angeles, CA, 90028', 'Hollywood Blvd and Vine St, Los Angeles, CA, 90028', 'Mission Street at Valencia Street, San Francisco, CA, 90028']
    parseable.each do |location|
      expect(described_class.parse(location)).to(be_truthy)
    end
  end

  it('comparison') do
    addr = described_class.parse(ADDRESSES.first[0])
    expect(addr.to_s).to(eq('1005 Gravenstein Hwy, 95472'))
    expect(described_class.parse(ADDRESSES.first[0])).to(eq(addr))
    expect(described_class.parse(EXPECTED_FAILURES.first)).not_to(eq(addr))
